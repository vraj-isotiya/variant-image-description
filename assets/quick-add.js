  ...
  customElements.define(
    ...
    class QuickAddModal extends ModalDialog {
      ...
      show(opener) {
        ...
        // Custom Code
        // Base product URL
        let productUrl = opener.getAttribute("data-product-url");

        // Find the selected variant input within the card
        const card = opener.closest(".card-product-custom-div");
        let selectedVariantInput = card?.querySelector(
          '.tr-collection-product-card__swatch input[type="radio"]:checked'
        );

        if (selectedVariantInput) {
          const variantId = selectedVariantInput.dataset.variantId;
          if (variantId) {
